...
-- record Bisim2 (s₁ s₂ : νPfin2) : Type where
--   coinductive
--   field
--     forceEq2 : PfinRel Bisim2 (force2 s₁) (force2 s₂)
-- open Bisim2
...
-- t1≡t2 : t2 ≡ t1
-- force2 (t1≡t2 i) = idem (η (t1≡t2 i)) i
...
-- force2 (ana-uniq c f eq x i) = {!!} --(ana-uniq' c f eq (c x) ∙ sym (eq x)) i
-- --  hcomp (doubleComp-faces (λ _ → ana' c (c x)) (λ j → eq x (~ j)) i) (ana-uniq' c f eq (c x) i)
-- ana-uniq' c f eq ø _ = ø
-- ana-uniq' c f eq (η x) i = η (ana-uniq c f eq x i)
-- ana-uniq' c f eq (s ∪ s₁) i = ana-uniq' c f eq s i ∪ ana-uniq' c f eq s₁ i --cong₂ _∪_ (ana-uniq' c f eq s) (ana-uniq' c f eq s₁)
-- ana-uniq' c f eq (com s s₁ i) j = com (ana-uniq' c f eq s j) (ana-uniq' c f eq s₁ j) i
-- ana-uniq' c f eq (ass s s₁ s₂ i) j = ass (ana-uniq' c f eq s j) (ana-uniq' c f eq s₁ j) (ana-uniq' c f eq s₂ j) i
-- ana-uniq' c f eq (idem s i) j = idem (ana-uniq' c f eq s j) i
-- ana-uniq' c f eq (nr s i) j = nr (ana-uniq' c f eq s j) i
-- ana-uniq' c f eq (trunc s s₁ x y i j) k =
--   trunc (ana-uniq' c f eq s k) (ana-uniq' c f eq s₁ k) (λ l → ana-uniq' c f eq (x l) k) (λ l → ana-uniq' c f eq (y l) k) i j

-- open import Cubical.Data.Nat

-- ωChain : Type₁
-- ωChain = Σ (ℕ → Type) λ X → ∀ n → X (suc n) → X n

-- ωCone : ωChain → Type₁
-- ωCone (X , r) =
--   Σ Type (λ Z → Σ (∀ n → Z → X n) λ f → ∀ n z → r n (f (suc n) z) ≡ f n z)

-- ωLimit : ωChain → Type₁
-- ωLimit Ch =
--   Σ (ωCone Ch) λ Co → (Co' : ωCone Ch) → 
--     Σ (Co' .fst → Co .fst) λ ζ →
--       (∀ n z → Co .snd .fst n (ζ z) ≡ Co' .snd .fst n z)
--       ×
--       ((ζ' : Co' .fst → Co .fst) (p' : ∀ n z → Co .snd .fst n (ζ' z) ≡ Co' .snd .fst n z) → ζ ≡ ζ')

-- mapPfin∘ : ∀{A B C} (f : B → C) (g : A → B) (x : Pfin A)
--   → mapPfin f (mapPfin g x) ≡ mapPfin (\ x → f (g x)) x
-- mapPfin∘ f g ø = refl
-- mapPfin∘ f g (η x) = refl
-- mapPfin∘ f g (s ∪ s₁) i = mapPfin∘ f g s i ∪ mapPfin∘ f g s₁ i
-- mapPfin∘ f g (com x x₁ i) j = com (mapPfin∘ f g x j) (mapPfin∘ f g x₁ j) i
-- mapPfin∘ f g (ass x x₁ x₂ i) j = ass (mapPfin∘ f g x j) (mapPfin∘ f g x₁ j) (mapPfin∘ f g x₂ j) i
-- mapPfin∘ f g (idem x i) j = idem (mapPfin∘ f g x j) i
-- mapPfin∘ f g (nr x i) j = nr (mapPfin∘ f g x j) i
-- mapPfin∘ f g (trunc m0 m1 x y i i') j = trunc (mapPfin∘ f g m0 j) (mapPfin∘ f g m1 j)
--                                               (cong (\ x → mapPfin∘ f g x j) x) (cong (\ x → mapPfin∘ f g x j) y) i i'

-- module _ (Ch : ωChain)(Lim : ωLimit Ch) where

--   X = Ch .fst
--   r = Ch .snd
--   L = Lim .fst .fst
--   c = Lim .fst .snd .fst
--   c-tr = Lim .fst .snd .snd
--   is-lim = Lim .snd
  
--   Pfin-Ch : ωChain
--   Pfin-Ch = (λ n → Pfin (X n)) , (λ n → mapPfin (r n))

--   Pfin-Co : ωCone Pfin-Ch
--   Pfin-Co = Pfin L , (λ n → mapPfin (c n)) , λ n z → mapPfin∘ (r n) (c (suc n)) z ∙ (λ i → mapPfin (λ x → c-tr n x i) z)
  
--   Pfin-Lim : ωLimit Pfin-Ch
--   Pfin-Lim = Pfin-Co ,
--     λ { Co' → {!is-lim!} , {!!} , {!!} }
...
-- -- -- bisim2' : (t₁ t₂ : Pfin νPfin2) → PfinDRel Bisim2 t₁ t₂ → PfinDRel _≡_ t₁ t₂
...
-- -- --   SameEq→Eq (force2 t₁) (force2 t₂) (bisim2' (force2 t₁) (force2 t₂) (forceEq2 b .fst) , bisim2' (force2 t₂) (force2 t₁) (forceEq2 b .snd)) i
...
-- -- --   coinductive
-- -- --   field
-- -- --     forceEq2 : {k : Size< j} → PfinRel (Bisim2 j k) (force2 s₁) (force2 s₂)
-- -- -- open Bisim2

-- -- -- bisim2 : ∀{j}{k : Size< j} (t₁ t₂ : νPfin2 j) → Bisim2 j k t₁ t₂ → t₁ ≡ t₂
-- -- -- bisim2' : ∀{j}{k : Size< j} (t₁ t₂ : Pfin (νPfin2 j)) → PfinDRel (Bisim2 j k) t₁ t₂ → PfinDRel _≡_ t₁ t₂ 
-- -- -- force2 (bisim2 t₁ t₂ b i) =
-- -- --   SameEq→Eq (force2 t₁) (force2 t₂) ({!bisim2' (force2 t₁) (force2 t₂) ?!} , {!!}) {!!}
-- -- -- --  SameEq→Eq (force2 t₁) (force2 t₂) (bisim2' (force2 t₁) (force2 t₂) (forceEq2 b .fst) , bisim2' (force2 t₂) (force2 t₁) (forceEq2 b .snd)) i
-- -- -- bisim2' t₁ t₂ b x mx = ∥map∥ (λ { (y , my , eq) → y , my , bisim2 x y eq}) (b x mx)
-- -- -- -}
...
-- -- -- -- -- bisim : (t₁ t₂ : νPfin) → Bisim t₁ t₂ → t₁ ≡ t₂ --PfinDRel _≡_ (force t₁) (force t₂)
-- -- -- -- -- bisim' : (t₁ t₂ : Pfin νPfin) → PfinDRel Bisim t₁ t₂ → PfinDRel _≡_ t₁ t₂ --{x : νPfin} → ⟨ x ∈ₛ t₁ ⟩ → ⟨ {!!} ⟩ 
-- -- -- -- -- force (bisim t₁ t₂ b i) = {!!}
-- -- -- -- -- --  PfinDRelEq (force t₁) (force t₂) (bisim' (force t₁) (force t₂) (forceEq b)) i
...
-- -- -- -- -- -- ana-uniq : ∀{X} (c : X → Pfin X)
-- -- -- -- -- --   → (f : X → νPfin) (eq : ∀ x → PfinDRel _≡_ (force (f x)) (mapPfin f (c x)))
-- -- -- -- -- --   → ∀ x → Bisim (ana c x) (f x)
