...
-- existence-part of the universal property of the limit: given a
-- setoid L which is a cone, then there exists a setoid morphism from
-- L to the limit
...
-- the limit of the ω-chain iPfinS-ch
ωPfinS : Setoid₀
...
mS : PfinS ωPfinS →S ωPfinS
mS = ∃upωLimit iPfinS-ch _ isConePfinSωPfinS

-- the (n+1)-projections of the image of mS have fixed length,
-- given by the length of the input xs
lengthAlgωPfinS : ∀ xs n → length (mS .mor xs .fst (suc n)) ≡ length xs
...
iPfinS2 : ℕ → Setoid₀ 
iPfinS2 zero = ωPfinS
iPfinS2 (suc n) = PfinS (iPfinS2 n)

iMapPfinS2 : ∀ n → iPfinS2 (suc n) →S iPfinS2 n
iMapPfinS2 zero = mS
iMapPfinS2 (suc n) = mapPfinS (iMapPfinS2 n)

iPfinS2-ch : ωChainS₀
iPfinS2-ch = iPfinS2 , iMapPfinS2

-- some convenient synonyms
iList2 : ℕ → Type
iList2 n = iPfinS2 n .Carr

iListRel2 : (n : ℕ) (xs ys : iList2 n) → Type
iListRel2 n = iPfinS2 n .Rel

reflIList2 : (n : ℕ) {xs : iList2 n} → iListRel2 n xs xs
reflIList2 n = iPfinS2 n .eqrRel .reflexive _

symIList2 : (n : ℕ) {xs ys : iList2 n}
  → iListRel2 n xs ys → iListRel2 n ys xs 
symIList2 n = iPfinS2 n .eqrRel .symmetric _ _

transIList2 : (n : ℕ) {xs ys zs : iList2 n}
  → iListRel2 n xs ys → iListRel2 n ys zs 
  → iListRel2 n xs zs
transIList2 n = iPfinS2 n .eqrRel .transitive _ _ _

iMapList2 : ∀ n → iList2 (suc n) → iList2 n
iMapList2 n = iMapPfinS2 n .mor

iMapListRel2 : ∀ n {xs ys : iList2 (suc n)}
  → Relator (iListRel2 n) xs ys → iListRel2 n (iMapList2 n xs) (iMapList2 n ys)
iMapListRel2 n = iMapPfinS2 n .morRel 

-- the limit of the ω-chain iPfinS-ch
ωPfinS2 : Setoid₀
ωPfinS2 = ωLimitS iPfinS2-ch

uS : (n : ℕ) → iPfinS2 n →S ωPfinS
uS zero = idS
uS (suc n) = uS n ∘S iMapPfinS2 n

∩u : Type
∩u = Σ[ x ∈ ((n : ℕ) → iList2 n)] (∀ n → ωPfinS .Rel (uS (suc n) .mor (x (suc n))) (uS n .mor (x n)))
...
1) Prove that mS is injective
   (Seems to require countable choice!)

2) Prove that uS n is injective for all n

3) Show that ωPfinS2 is isomorphic to ∩uS

4) Show that PfinS preserves ω-intersections, in particular ∩uS
   (using countable choice)

5) Conclude that PfinS ωPfinS2 is isomorphic to the (ω+ω)-limit of the
shifted chain

PfinS ωPfinS, PfinS^2 ωPfinS, Pfin^3 ωPfinS, ...

which is in turn isomorphic to ωPfinS2
...
-- -- that the algebra mS is not surjective
...
-- -- We show that the algebra mS is not surjective by giving an
-- -- element of ωPFinS which is not in the image of mS (This is a
...
--   noSurjAlgωPfinS' : (xs : PfinS ωPfinS .Carr) → ωPfinS .Rel (mS .mor xs) growingS → ⊥
--   noSurjAlgωPfinS' xs p = ¬m<m absurd
--     where
--       N : ℕ
--       N = length xs
  
--       zs = mS .mor xs .fst (suc N)
--       ys = growing (suc N)
...
--   noSurjAlgωPfinS : isSurjectionS mS → ⊥
